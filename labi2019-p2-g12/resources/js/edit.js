function renderObjects()
{
    ...
    $.getJSON('https://xcoa.av.it.pt/labi2019-p2-g12//get?id='+id, 
        function(data) 
        {
            let image_html = document.getElementById("img")
            let inputs = document.getElementById("inputs_edit")
            if(data == "not found"){
                image_html.innerHTML = `<h6 class="text-center"><img width="45px" src = "/labi2019-p2-g12/icons/not_Found.png"></h6>`
                inputs.innerHTML =
                `<h4>Image not Found</h4>
                `
            }
            else{
                image_html.innerHTML = `<h6 class="text-center"><img class="rounded" width="200px" src = "/labi2019-p2-g12/images/objs/` + data[0]['name'] + `"></h6>`
                inputs.innerHTML =
...
function save()
{
    ...
    $.getJSON('https://xcoa.av.it.pt/labi2019-p2-g12//edit_object?id=' + id + `&obj=` + text +`&confidence=` + confidence,
        function(data) 
        {
            renderObjects()
            alert(data)
